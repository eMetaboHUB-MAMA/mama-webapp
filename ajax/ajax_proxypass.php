...
if (isset($_GET["resource"]) && $_GET["resource"] != "") {
	switch ($_GET["resource"]) {
		...
		case "managerkeyword":
			...
			if ($verbe == "PUT") {
				put_managerkeyword($mama_config_json['mamaRestApi']);
			}
			if ($verbe == "DELETE") {
				// get_server_load ( $mama_config_json ['mamaRestApi'] );
			}
		...
		case "mth-sub-platform":
			...
			if ($verbe == "PUT") {
				put_mthSubPlatform($mama_config_json['mamaRestApi']);
			}
			if ($verbe == "DELETE") {
				// nope!
			}
...
function put_managerkeyword($mama_rest_url)
{
	$keywordID = "";
	if (isset($_GET['keywordID']) && $_GET['keywordID'] != "") {
		$keywordID = $_GET['keywordID'];
	}
	$curl_response = getStandardizedPut($mama_rest_url, "managerkeyword/" . $keywordID);
	if ($curl_response == "null") {
		returnSuccess(false);
	}
	// return
...
function get_managerkeywords($mama_rest_url)
{
	$curl_response = getStandardizedGet($mama_rest_url, "managerkeywords");
	echo $curl_response;
}

function post_managerkeyword($mama_rest_url)
{
	$curl_response = getStandardizedPost($mama_rest_url, "managerkeyword");
	if ($curl_response == "null") {
		returnSuccess(false);
	}
	// return
...
function get_mthSubPlatforms($mama_rest_url)
{
	$curl_response = getStandardizedGet($mama_rest_url, "mth-sub-platforms");
	echo $curl_response;
}

function post_mthSubPlatform($mama_rest_url)
{
	$curl_response = getStandardizedPost($mama_rest_url, "mth-sub-platform");
	if ($curl_response == "null") {
		returnSuccess(false);
	}
	// return
...
function put_mthSubPlatform($mama_rest_url)
{
	$mthSubPlatformID = "";
	if (isset($_GET['mthSubPlatformID']) && $_GET['mthSubPlatformID'] != "") {
		$mthSubPlatformID = $_GET['mthSubPlatformID'];
	}
	$curl_response = getStandardizedPut($mama_rest_url, "mth-sub-platform/" . $mthSubPlatformID);
	if ($curl_response == "null") {
		returnSuccess(false);
	}
	// return
